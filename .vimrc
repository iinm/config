...
if executable('rg')
  set grepprg=rg\ --vimgrep\ --hidden\ --glob\ '!.git'\ --glob\ '!node_modules'
endif
set splitbelow
set splitright
...
nnoremap - :<C-u>e %:h <bar> /<C-r>=expand('%:t')<CR><CR>
...
nnoremap <leader>o :<C-u>Oldfiles<CR>
...
command -nargs=0 Oldfiles call Oldfiles({'only_cwd': v:true})
command -nargs=0 OldfilesGlobal call Oldfiles()
...
augroup vimrc_update_oldfiles
  autocmd!
  autocmd BufReadPost * call UpdateOldfiles(expand('<afile>:p'))
augroup END
